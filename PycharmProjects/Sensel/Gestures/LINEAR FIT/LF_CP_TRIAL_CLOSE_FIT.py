...
def findpoints(xdata, ydata):
    if len(points_seen_till_now) > 1:
        ...
        print("QS", querystart)
        print("QE", queryend)
        print("Query 1", points_seen_till_now[querystart[1]][0])
        print("Query 2", points_seen_till_now[queryend[1]][0])
        print("Query 3", points_seen_till_now[querystart[1]][1])
        print("Query 4", points_seen_till_now[queryend[1]][1])

        if points_seen_till_now[querystart[1]][0] == points_seen_till_now[queryend[1]][0] and \
                        points_seen_till_now[querystart[1]][1] == points_seen_till_now[queryend[1]][1]:
            if querystart[0] < queryend[0] and querystart[0] < 25:
                print("CASE1")
                xdata[0] = points_seen_till_now[querystart[1]][0]
                ydata[0] = points_seen_till_now[querystart[1]][1]
            elif querystart[0] > queryend[0] and queryend[0] < 25:
                print("CASE2")
                xdata[len(xdata) - 1] = points_seen_till_now[queryend[1]][0]
                ydata[len(xdata) - 1] = points_seen_till_now[queryend[1]][1]
        elif querystart[0] < 25:
            print("CASE3")
            xdata[0] = points_seen_till_now[querystart[1]][0]
            ydata[0] = points_seen_till_now[querystart[1]][1]
            if queryend[0] < 25:
